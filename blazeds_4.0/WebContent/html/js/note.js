...
function createFlex() {
    ...
    swfobject.embedSWF(
        "/html/flex/MunjaNote.swf?v=20140219", "flashContent",
        "1024", "740",
...
function apiTab() {
    ...
    if (web.hasClass("apiTabOn")) { web.removeClass("apiTabOn");$("#apiWebHelp").hide(); }
    else { web.addClass("apiTabOn"); $("#apiWebHelp").show(); }
...
function cash_req(f) {
    ...
    if (cn == "") alert("입금자명을 입력해 주세요.");
        ...
        if (confirm(cn+" 으로 입금 예약 하시겠습니까?")) {
            ...
            $.getJSON("/mypage/_cash.jsp",$(f).serialize(),
                function(data) {
                    if (data != null && data.rslt*1 > 0) {
...
function joinSubmit() {
    ...
    var err = joinValid();
    if (err == "") {
        if (confirm("입력하신 정보로 가입 하시겠습니까?\n\n※ 게임&도박&성인 관련 스팸 발송 시 사용자 동의 없이 아이디 차단 될수 있으니 유념하시기 바랍니다.")) {
