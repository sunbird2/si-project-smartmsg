...
try {
	...
	if (lines != null && lines.size() > 0) {
		...
		int totalcnt = 0;
		StatusVO tmp = null;
		cnt = sm.updateSmrelaySMS(svo);
		VbyP.accessLog("######### failAdd Client : dt="+svo.getDt()+" cnt="+cnt);
		for (int i = 0; i < cntLines; i++) {
			...
			if (!tmp.getDt().equals("")) {
				...
				if (cnt > 0 && tmp.getSms() > 0) {
				   ...
				   if (rsltcnt < 1)
					   VbyP.accessLog("failAdd Client update : user_id="+tmp.getDt()+" rsltcnt="+rsltcnt);
				} // if
			}
