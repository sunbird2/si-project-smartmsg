...
import com.m.url.Szip;
import com.m.url.SzipVO;


public class SendUrlManager implements ISend, MybatisAble {
	...
	public LogVO send(Connection conn, UserInformationVO uvo, SendMessageVO smvo)
		...
		int typeUrl =  SLibrary.intValue(VbyP.getValue("URL_COUNT"));
		int point = smvo.getAl().size() * typePoint;
		int pointUrl = smvo.getAl().size() * typeUrl;
		
		if (idx > 0)
			rslt = updatePoint(conn, uvo, lvo.getMode(), point);
		else throw new Exception("insertLog Error!!");
		// url point 
		if (rslt > 0) rslt = updatePointUrl(conn, uvo, lvo.getMode(), pointUrl);
		else throw new Exception("insertPoint Error!!");
		...
		if (rslt > 0){
			...
			if (rslt*typePoint < point) {
				...
				rslt = updateFailPoint(conn, uvo, lvo.getMode(), point - (rslt*typePoint)+rslt);
				if ( rslt <= 0 ) VbyP.errorLog(uvo.getUser_id()+" >> fail add : "+ (point - (rslt*typePoint+rslt))+" "+lvo.getMode()+" Fail!!!!!");
			}
	...
	public int updatePoint(Connection conn, UserInformationVO uvo, String type, int count) throws Exception {
		...
		int code = 0;
		if (type.equals("LMS")) { code = SendUrlManager.LMS_CODE;}
		else if (type.equals("MMS")) { code = SendUrlManager.MMS_CODE; }
		else { code = SendUrlManager.SMS_CODE; }
	...
	private int updatePointUrl(Connection conn, UserInformationVO uvo, String type, int count) throws Exception {
		
		int code = SendUrlManager.URL_CODE;
	...
	public int insertData(Connection conn, String mode, UserInformationVO uvo, ArrayList<MessageVO> al, String line) throws Exception {
		...
		if (count > 0) {
			...
			SessionManager sm = new SessionManager(sqlMapper.openSession(ExecutorType.REUSE ,true));
			sm.setbClose(false);
			try {
				for (int i = 0; i < count; i++) {
					...
					if (!Refuse.isRefuse(hashTable_refuse, vo.getPhone()) && !hashTable_duple.containsKey(vo.getPhone())) {
						...
						resultCount += sm.insert(ns + "insert_url_send_"+line+"_"+mode, vo);
					}
					Gv.setStatus(uvo.getUser_id(), Integer.toString(i+1));
				}
			
			} catch (Exception e) {
				System.out.println("SendUrlManager Exception!! : "+e);
			} finally {
				sm.close();
			}
		}
	...
	private ArrayList<MessageVO> makeMessageVOArrayList(UserInformationVO uvo, SendMessageVO smvo, int groupKey) throws Exception {
		...
		try {
			for (int i = 0; i < count; i++) {
				...
				vo.setIdx(smvo.getUrlKey());
				
				if (!Refuse.isRefuse(hashTable_refuse, vo.getPhone()) && !hashTable_duple.containsKey(vo.getPhone())) {
					hashTable_duple.put(vo.getPhone(), "");
					sm.insert(ns + "insert_url_data", vo);
					
					vo.setMsg(getUrlMsg(smvo.getMessage(), getUrl(uvo.getLine(), vo.idx)));
				}
	...
	private String getMode(SendMessageVO smvo) {
		...
		if (!SLibrary.isNull(smvo.getImagePath()))
			mode = "MMS";
		else if ( SLibrary.getByte( smvo.getMessage() ) > SendUrlManager.SMS_BYTE)
			mode = "LMS";
		return mode;
	}
	...
	private String getUrlMsg(String msg, String url) {
		
		String rslt = msg;
		rslt = SLibrary.replaceAll(rslt, MEARGE_URL, " "+url+" " );
		return rslt;
	}
	private String getUrl(String line, int idx) throws Exception {
		
		String ls = "";
		if (line.equals("lg")) ls = "l";
		else if (line.equals("kt")) ls = "k";
		else if (line.equals("han")) ls = "h";
		else if (line.equals("lgspam")) ls = "s";
		else throw new Exception("no line class!!");
		...
		vo.setKey(ls);
		vo.setIndex(idx);
		System.out.println("encode = "+Szip.getEncode(vo));
		
		
		return URL+Szip.getEncode(vo);
	}
